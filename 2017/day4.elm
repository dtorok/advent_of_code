...
solver wordTrans input = input
  |> parseRows -- parsed pass phrases
  |> List.map (List.map wordTrans)
  |> List.map isPassphraseValid
  |> List.filter (\x -> x == True)
  |> List.length
  |> toString

isPassphraseValid : List String -> Bool
isPassphraseValid words =
  let
    f : Set String -> List String -> Bool
    f cache words =
      case words of
        w :: ws ->
          if Set.member w cache then
            False
          else
            f (Set.insert w cache) ws
        [] -> True
  in
    f Set.empty words
