...
part1 : TestRun.Test
part1 =
  { title = "Day 5: A Maze of Twisty Trampolines, All Alike - Part 1"
  , solver = solver incUpdater
  , testCases =
    [ ( smallInput, "5")
    , ( bigInput, "396086")
    ]
...
part2 : TestRun.Test
part2 =
  { title = "Day 5: A Maze of Twisty Trampolines, All Alike - Part 2"
  , solver = solver around3Updater
  , testCases =
    [ ( smallInput, "10")
    -- , ( bigInput, "28675390") -- too long to run
    ]
  }

-- SOLVERS
----------
solver : (Int -> Int) -> String -> String
solver offsetUpdater input = input
  |> parseInstructions -- Array Int
  |> Model 0 0
  |> runInstructions offsetUpdater
  |> (\(Model counter _ _) -> counter)
  |> toString

-- HELPERS
----------
type Model = Model Int Int (Array Int)

runInstructions : (Int -> Int) -> Model -> Model
runInstructions offsetUpdater (Model counter pos instructions) =
  let
    maybeOffset = Array.get pos instructions
  in
    case maybeOffset of
      Nothing ->
        Model counter pos instructions
      Just offset ->
        runInstructions offsetUpdater (
          Model
            (counter + 1)
            (pos + offset)
            (Array.set pos (offsetUpdater offset) instructions)
        )

incUpdater : Int -> Int
incUpdater a = a + 1

around3Updater : Int -> Int
around3Updater a =
  if a >= 3
  then a - 1
  else a + 1
