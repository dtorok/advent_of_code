...
analyzeStream model stream = List.foldl analyzeChar model stream

analyzeChar : Char -> Model -> Model
analyzeChar char model =
  case model.state of
    Normal -> analizeNormalChar char model
    Garbage -> analizeGarbageChar char model
    Escape -> analizeEscapeChar char model

analizeNormalChar : Char -> Model -> Model
analizeNormalChar char model =
  case char of
    '{' ->
      { model
      | level = model.level + 1
      , score = model.score + model.level + 1
      }
    '}' ->
      { model
      | level = model.level - 1
...
analizeGarbageChar : Char -> Model -> Model
analizeGarbageChar char model =
  case char of
    '>' ->
      { model
      | state = Normal
