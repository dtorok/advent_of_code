...
part1 : TestRun.Test
part1 =
  { title = "Day 8: I Heard You Like Registers - Part 1"
  , solver = solver findLargestValue
  , testCases =
    [ (smallInput, "1")
    , (bigInput, "6343")
    ]
...
part2 : TestRun.Test
part2 =
  { title = "Day 8: I Heard You Like Registers - Part 2"
  , solver = solver getMaxVal
  , testCases =
    [ (smallInput, "10")
    , (bigInput, "7184")
    ]
...
-- SOLVER
---------
solver : (Memory -> Maybe Int) -> String -> String
solver extractor input = input
  |> parseInput -- List Expression
  |> runInstructions Dict.empty -- Memory
  |> extractor -- Maybe Int
  |> Maybe.map toString
  |> Maybe.withDefault "empty memory"

-- INSTRUCTION
--------------
type Instruction = Inc | Dec
type Operator = LT | LE | EQ | NE | GE | GT
...
runInstructions : Memory -> List Expression -> Memory
runInstructions memory expressions =
  let
    eval : Int -> Operator -> Int -> Bool
    eval val1 op val2 =
      case op of
        LT -> val1 < val2
        LE -> val1 <= val2
    ...
    evaluateExpression : Expression -> Memory -> Bool
    evaluateExpression expr mem =
      let
        regVal = Dict.get expr.condReg mem |> Maybe.withDefault 0
      in
        eval regVal expr.op expr.operand

    apply : Int -> Instruction -> Int -> Int
    apply val instr offset =
      case instr of
        Inc -> val + offset
        Dec -> val - offset

    applyExpression : Expression -> Memory -> Memory
    applyExpression expr memory =
      let
        regVal = Dict.get expr.reg memory |> Maybe.withDefault 0
        newRegVal = apply regVal expr.instr expr.offset
        newMaxVal = memory
          |> Dict.get maxKey -- Maybe Int
          |> Maybe.withDefault newRegVal
          |> max newRegVal -- Int
      in
        memory
          |> Dict.insert expr.reg newRegVal
          |> Dict.insert maxKey newMaxVal

    run : Expression -> Memory -> Memory
    run expr memory =
      if evaluateExpression expr memory
      then applyExpression expr memory
      else memory
  in
    List.foldl run memory expressions

findLargestValue : Memory -> Maybe Int
findLargestValue memory = memory
  |> Dict.remove maxKey
  |> Dict.values -- List Int
  |> List.maximum -- Mayb Int

getMaxVal : Memory -> Maybe Int
getMaxVal memory = Dict.get maxKey memory

-- PARSER
---------
parseInput : String -> List Expression
parseInput input = input
  |> String.trim -- String
  |> String.split "\n" -- List String
  |> List.filterMap parseExpression -- List Expression

parseExpression : String -> Maybe Expression
parseExpression line = line
  |> String.trim -- String
  |> Regex.find Regex.All (Regex.regex "(\\w+) (\\w+) ([-\\d]+) if (\\w+) ([!<>=]+) ([-\\d]+)") -- List Regex.Match
  |> List.head -- regex.Match
  |> Maybe.map .submatches -- List String
  |> Maybe.andThen makeExpression -- Maybe Expression

makeExpression : List (Maybe String) -> Maybe Expression
makeExpression submatches =
  case submatches of
    Just reg :: Just strInstr :: Just strOffset :: Just condReg :: Just strOp :: Just strOperand :: [] ->
      let
        maybeInstr : Maybe Instruction
        maybeInstr = parseInstr strInstr

        maybeOffset : Maybe Int
        maybeOffset = parseInt strOffset

        maybeOp : Maybe Operator
        maybeOp = parseOperator strOp

        maybeOperand : Maybe Int
        maybeOperand = parseInt strOperand
      in
        case (maybeInstr, maybeOffset, maybeOp, maybeOperand) of
          (Just instr, Just offset, Just op, Just operand) -> Just
            { reg     = reg
            , instr   = instr
            , offset  = offset
            , condReg = condReg
